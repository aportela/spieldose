...
class Album
{
    ...
    public function save(\aportela\DatabaseWrapper\DB $dbh)
    {
        ...
        $params = array(
            ...
            new \aportela\DatabaseWrapper\Param\StringParam(":album", $this->album)
        );
        if (!empty($this->artist)) {
            $params[] = new \aportela\DatabaseWrapper\Param\StringParam(":artist", $this->artist);
        } else {
        ...
        $dbh->exec("REPLACE INTO MB_CACHE_ALBUM (MBID, ALBUM, ARTIST, IMAGE, JSON) VALUES (:mbid, :album, :artist, :image, :json)", $params);
    }
}
