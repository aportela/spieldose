...
class Artist
{
    ...
    const MUSICBRAINZ_API_BASE_URL = "http://musicbrainz.org/ws/2/artist/?query=artist:%s&limit=%d&fmt=json";
    ...
    public static function searchMusicBrainzId(string $name = "", int $limit = 1): array
    {
        $url = sprintf(self::MUSICBRAINZ_API_BASE_URL, urlencode($name), $limit);
        $result = \Spieldose\Net::httpRequest($url, self::USER_AGENT);
        $result = json_decode($result);
        $mbIds = array();
    ...
    public static function getFromArtist(string $artist)
    {
        if (empty($artist)) {
            ...
            $url = sprintf(self::API_GET_URL_FROM_NAME, \Spieldose\LastFM::API_KEY, urlencode($artist));
            $json = \Spieldose\Net::httpRequest($url, self::USER_AGENT);
            $result = json_decode($json, false);
            if (!isset($result->error)) {
                $image = isset($result->artist->image) ? self::getBestImage($result->artist->image) : "";
    ...
    public function save(\aportela\DatabaseWrapper\DB $dbh)
    {
        ...
        if (!empty($this->bio)) {
            $params[] = new \aportela\DatabaseWrapper\Param\StringParam(":bio", $this->bio);
        } else {
            $params[] = new \aportela\DatabaseWrapper\Param\NullParam(":bio");
        }
        if (!empty($this->image)) {
        ...
        $dbh->exec("REPLACE INTO MB_CACHE_ARTIST (MBID, ARTIST, BIO, IMAGE, JSON) VALUES (:mbid, :artist, :bio, :image, :json)", $params);
    }
}
