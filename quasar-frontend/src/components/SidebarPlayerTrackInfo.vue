<template>
  <div id="song_info" class="q-px-md" style="max-width: 400px;" v-if="currentElement && currentElement.track">
    <p class="song_info_track text-center text-weight-bolder ellipsis" style="color: #d30320;" :title="currentElement.track.title || '&nbsp;'">{{ currentElement.track.title || null }}</p>
    <p class="song_info_album text-center ellipsis" :title="currentElement.track.album.title || null">{{ currentElement.track.album.title || "&nbsp;" }}</p>
    <p class="song_info_artist text-center ellipsis">
      <router-link v-if="currentElement.track.artist.name" style="text-decoration: none;"
        :to="{ name: 'artist', params: { name: currentElement.track.artist.name } }" :title="currentElement.track.artist.name">{{ currentElement.track.artist.name }}</router-link>
      <span v-else>&nbsp;</span>
    </p>
  </div>
  <div id="song_info" v-else-if="currentElement && currentElement.radioStation">
    <p class="song_info_track text-center text-weight-bolder ellipsis" style="color: #d30320;" :title="currentElement.radioStation.name">{{ currentElement.radioStation.name || "&nbsp;" }}</p>
