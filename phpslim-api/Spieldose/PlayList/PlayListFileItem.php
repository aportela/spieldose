...
final class PlayListFileItem extends PlayListItem
{
    ...
    public static function getPlayListFileItems(\aportela\DatabaseWrapper\DB $db, string $playListId): array
    {
        $results = $db->query(
            "
                SELECT
                ...
                FROM FILE
                INNER JOIN PLAYLIST_FILE ON PLAYLIST_FILE.file_id = FILE.id
    ...
    public static function getFavoritesPlayListFileItems(\aportela\DatabaseWrapper\DB $db): array
    {
        $results = $db->query(
            "
                SELECT
                ...
                LEFT JOIN FILE_ID3_TAG ON FILE_ID3_TAG.file_id = FILE.id
                LEFT JOIN DIRECTORY ON DIRECTORY.id = FILE.directory_id
                INNER JOIN FILE_FAVORITE ON FILE_FAVORITE.file_id = FILE.id AND FILE_FAVORITE.user_id = :user_id
                ORDER BY FILE_FAVORITE.ftime
            ",
            [
                new \aportela\DatabaseWrapper\Param\StringParam(":default_mime", "application/octet-stream"),
                new \aportela\DatabaseWrapper\Param\StringParam(":user_id", \Spieldose\UserSession::getUserId()),
